...
list<string> BancoDados::executa_statement() throw(BancoDadosExcecao){
list<string> listaResultado;
BancoDadosExcecao *erroExecutaStatement = new BancoDadosExcecao;

    try {
        if (sqlite3_step(this->statement) != SQLITE_DONE && sqlite3_step(this->statement) != SQLITE_ROW){
        throw *erroExecutaStatement;
        }
    ...
    catch(BancoDadosExcecao &erroExecutaStatement){
    erroExecutaStatement.erro_banco_dados(this->database);
    }
delete(erroExecutaStatement);
